/*
 * Copyright Andreas Samoljuk.
 *
 * MIT-licensed.
 *
 * Name:          fptostr.c
 * Description:   Convert a double into a string representation.
 * Author:        Andreas Samoljuk.
 *
 * This is an implementation of Fabian Loitsch's Grisu2 algorithm. The
 * reference Grisu2 code is actually a bit faster, but this is more
 * portable, and cleaner.
 *
 * See: https://github.com/night-shift/fpconv for the Samoljuk
 * code, and http://florian.loitsch.com/publications/dtoa-pldi2010.pdf
 * for the original Loitsch paper (though the link is now dead it seems?)
 */
...
static void
get_normalized_boundaries(Fp *fp, Fp *lower, Fp *upper)
{
...
static void
round_digit(char *digits, int ndigits, uint64_t delta, uint64_t rem, uint64_t kappa, uint64_t frac)
{
...
static int
generate_digits(Fp *fp, Fp *upper, Fp *lower, char *digits, int *K)
{
...
static int
grisu2(double d, char *digits, int *K)
{
    Fp w = build_fp(d);
...
static int
filter_special(double fp, char *dest)
{
    if (fp == 0.0) {
        dest[0] = '0';
        return 1;
...
int
n00b_internal_fptostr(double d, char dest[24])
{
    char digits[18];
