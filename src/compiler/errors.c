...
n00b_format_error_message(n00b_compile_error *one_err, bool add_code_name)
{
    ...
    if (info->takes_args) {
        result = n00b_cstr_array_format(info->message,
                                       one_err->num_args,
                                       one_err->msg_parameters);
    }
    else {
        result = n00b_rich_lit(info->message);
    }

    if (add_code_name) {
        n00b_utf8_t *code = n00b_cstr_format(" ([em]{}[/])",
                                           n00b_new_utf8(info->name));
        result           = n00b_str_concat(result, code);
    }

    return result;
}

static n00b_utf8_t *error_constant = NULL;
static n00b_utf8_t *warn_constant  = NULL;
static n00b_utf8_t *info_constant  = NULL;
...
n00b_format_runtime_errors(n00b_list_t *errors)
{
    ...
    n00b_grid_t *table = n00b_new(n00b_type_grid(),
                                n00b_kw("container_tag",
                                       n00b_ka(n00b_new_utf8("error_grid")),
                                       "td_tag",
                                       n00b_ka(n00b_new_utf8("tcol")),
                                       "start_cols",
                                       n00b_ka(3),
                                       "header_rows",
                                       n00b_ka(0)));
    ...
    for (int i = 0; i < n; i++) {
        ...
        n00b_utf8_t        *msg = n00b_format_error_message(err, true);

        n00b_list_append(row, format_severity(err));
        n00b_list_append(row, err->loc);
        n00b_list_append(row, msg);

        n00b_grid_add_row(table, row);
    }
...
static void
n00b_format_module_errors(n00b_module_t *ctx, n00b_grid_t *table)
{
    if (error_constant == NULL) {
        error_constant = n00b_rich_lit("[red]error:[/]");
        warn_constant  = n00b_rich_lit("[yellow]warning:[/]");
        info_constant  = n00b_rich_lit("[atomic lime]info:[/]");
    ...
    for (int i = 0; i < n; i++) {
        ...
        n00b_list_append(row, format_severity(err));
        n00b_list_append(row, err->loc);
        n00b_list_append(row, n00b_format_error_message(err, true));
...
n00b_format_errors(n00b_compile_ctx *cctx)
{
    ...
    n00b_utf8_t *snap        = n00b_new_utf8("full_snap");
    ...
    n00b_set_column_style(table, 0, snap);
    n00b_set_column_style(table, 1, snap);
...
#define COMMON_ERR_BASE()                                       \
    ...
    if (num_args) {                                             \
        for (int i = 0; i < num_args; i++) {                    \
            err->msg_parameters[i] = va_arg(args, n00b_str_t *); \
        }                                                       \
