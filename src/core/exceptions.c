...
n00b_default_uncaught_handler(n00b_exception_t *exception)
{
    ...
    n00b_stream_t     *errf = n00b_get_stderr();
    ...
    n00b_stream_write_object(errf, tbl, true);

#if defined(N00B_DEBUG) && defined(N00B_BACKTRACE_SUPPORTED)
    if (!exception->c_trace) {
        n00b_printf("[h1]No C backtrace available.");
    }
    else {
        n00b_printf("[h1]C backtrace:");
        n00b_print(exception->c_trace);
    }
#endif
}
...
{
    n00b_gc_register_root(&__exception_stack, sizeof(__exception_stack) / 8);
...
{
    n00b_print(n00b_repr_exception_stack_no_vm(n00b_new_utf8("FATAL ERROR:")));
}
