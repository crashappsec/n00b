...
try_once:
        ...
        while (cell_epoch < write_epoch) {
            ...
            if (CAS(&self->cells[ix], &expected, candidate)) {
                if (hatring_is_enqueued(expected.state) && self->drop_handler) {
        ...
        while (cell_epoch <= read_epoch) {
            ...
            if (CAS(&self->cells[ix], &expected, candidate)) {
                ...
                if (read_epoch > cell_epoch) {
                     ...
                     * clean-up.
                     */
                    if (self->drop_handler && hatring_is_enqueued(expected.state)) {
                        (*self->drop_handler)(expected.item);
                    }
                 ...
                 */
                if ((hatring_cell_epoch(saw.state) == read_epoch) && !self->drop_handler) {
...
hatring_view(hatring_t *self)
{
    ...
    while ((n < end) && (ret->next_ix < self->size)) {
        ...
        if (hatring_is_enqueued(cell.state) && (hatring_cell_epoch(cell.state) == n)) {
