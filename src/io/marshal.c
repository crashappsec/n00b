...
static n00b_list_t *
n00b_internal_pickle(n00b_pickle_ctx *ctx, void *addr)
{
    int64_t         offset;
...
static n00b_list_t *
n00b_unpickle_xform(n00b_stream_t *e, void *thunk, void *obj)
{
    n00b_unpickle_ctx *ctx    = thunk;
...
n00b_new_pickler(n00b_stream_t *e)
{
    ...
    // Map true memory address for heap-level  records to the offset of the
    // offset in the output heap. We use the "real" heap for this, which
    // ensures that, when incrementally marshaling, moved memory
    // references will get updated.
    cookie->memos           = n00b_dict(n00b_type_ref(), n00b_type_u64());
    cookie->base            = n00b_get_virtual_heap_start();
    cookie->offset          = cookie->base;
    cookie->last_offset_end = cookie->base;

#if defined(N00B_ADD_ALLOC_LOC_INFO)
    cookie->file_cache = n00b_new_unmanaged_dict(HATRACK_DICT_KEY_TYPE_CSTR,
                                                 false,
                                                 true);
#endif

    n00b_static_lock_init(cookie->lock);

    return res;
}

n00b_stream_filter_t *
n00b_new_unpickler(n00b_stream_t *e)
{
    ...
    cookie->partial    = NULL;
    cookie->vaddr_info = n00b_list(n00b_type_ref());
    n00b_static_lock_init(cookie->lock);
...
static void
n00b_automarshal_cb(void *ignored_event, void *v, n00b_list_t *l)
{
    n00b_list_append(l, v);
}

n00b_buf_t *
n00b_automarshal(void *obj)
{
    n00b_list_t   *results = n00b_list(n00b_type_buffer());
    n00b_stream_t *cb      = n00b_callback_open((void *)n00b_automarshal_cb,
                                           results);
    n00b_add_marshaling(cb);
    n00b_notifier_t *n = n00b_get_io_callback_exit_notifier(cb);
    n00b_write(cb, obj);
    n00b_wait(n, 0);
...
{
    n00b_list_t   *objs = n00b_list(n00b_type_ref());
    n00b_stream_t *cb   = n00b_callback_open((void *)n00b_automarshal_cb,
                                           objs);
    n00b_add_unmarshaling(cb);
    n00b_notifier_t *n = n00b_get_io_callback_exit_notifier(cb);

    N00B_TRY
    {
        n00b_write(cb, b);
        if (n00b_wait(n, 1000) == -1) {
            objs = NULL;
        }
    }
    N00B_EXCEPT
    ...
    switch (n00b_len(objs)) {
    case 0:
        return NULL;
    case 1:
        return n00b_list_get(objs, 0, NULL);
    default:
        return objs;
    }
}
