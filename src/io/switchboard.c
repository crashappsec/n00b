...
{
    printf("Party time!\n");
    return n00b_gc_alloc_mapped(n00b_party_t, N00B_GC_SCAN_ALL);
}
...
new_subscription()
{
    return n00b_gc_alloc_mapped(n00b_subscription_t, N00B_GC_SCAN_ALL);
}
              ...
              n00b_party_t       *write_to)
{
    ...
    if (read_from->n00b_party_type == N00B_PT_STRING) {
        ...
        assert(n00b_in_heap(subscription));
        ...
        assert(n00b_in_heap(read_from));
        assert(n00b_in_heap(r_fd_obj));
        assert(n00b_in_heap(subscription));
    }
