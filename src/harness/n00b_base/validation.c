...
static n00b_utf8_t *
line_strip(n00b_str_t *s)
{
    n00b_list_t *parts = n00b_str_split(s, n00b_new_utf8("\n"));

    for (int i = 0; i < n00b_list_len(parts); i++) {
        n00b_utf8_t *line = n00b_to_utf8(n00b_list_get(parts, i, NULL));
        n00b_utf8_t *item = n00b_to_utf8(n00b_str_strip(line));
...
static void
show_hex_dump_if_enabled(n00b_utf8_t *expected, n00b_utf8_t *actual)
{
#ifdef N00B_HEX_DUMP_ON_TEST_FAIL
    int elen = n00b_str_byte_len(expected);
    int alen = n00b_str_byte_len(actual);
    n00b_printf("[h1]Expected hex:");
    n00b_print(n00b_hex_dump(expected->data, elen));
    n00b_printf("[h1]Actual hex:");
    ...
    for (int i = 0; i < n; i++) {
        if (expected->data[i] != actual->data[i]) {
            n00b_printf("First difference at byte: {:x}", n00b_box_u64(i));
            return;
        }
    }

    assert(elen != alen);
    n00b_printf("First difference at byte: {:x}", n00b_box_u64(n));
                    ...
                    n00b_buf_t       *outbuf)
{
    if (kat->expected_output) {
        if (!outbuf || n00b_buffer_len(outbuf) == 0) {
            if (!n00b_str_codepoint_len(kat->expected_output)) {
...
empty_err:
            ...
            n00b_utf8_t *output = n00b_buf_to_utf8_string(outbuf);
            output             = n00b_to_utf8(n00b_str_strip(output));

            if (n00b_str_codepoint_len(output) == 0) {
                goto empty_err;
            }

            if (kat->is_hex) {
                output = n00b_str_to_hex(output, false);
            }

            n00b_utf8_t *expected = line_strip(kat->expected_output);
            n00b_utf8_t *actual   = line_strip(output);

            if (!n00b_str_eq(expected, actual)) {
                n00b_printf(
                    "[red]FAIL[/]: test [i]{}[/]: output mismatch.",
                    kat->path);
                n00b_printf("[h1]Expected output[/]\n");
                n00b_print(expected);
                n00b_printf("[h1]Actual[/]\n");
                n00b_print(actual);
...
next_comparison:;
    ...
    if (num_expected != num_actual) {
        ...
        for (int i = 0; i < num_expected; i++) {
            ...
            c2 = n00b_to_utf8(n00b_str_strip(c2));

            if (!n00b_str_eq(n00b_err_code_to_str(c1), c2)) {
