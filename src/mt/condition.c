...
#endif
{
    ...
    int64_t timeout           = 0;
    bool    abort_if_unlocked = true;
    bool    auto_unlock       = false;
    void   *wake_parameter    = NULL;
    void   *result;

    n00b_karg_only_init(cv);
    n00b_kw_int64("timeout", timeout);
    n00b_kw_int64("predicate", predicate);
    n00b_kw_bool("require_lock", abort_if_unlocked);
    n00b_kw_bool("auto_unlock", auto_unlock);
    n00b_kw_ptr("wake_param", wake_parameter);
...
#endif
{
    ...
    n00b_karg_only_init(cv);
    n00b_kw_int64("predicate", predicate);
    n00b_kw_int64("max", max);
    n00b_kw_ptr("value", value);
    n00b_kw_bool("all", all);
    n00b_kw_bool("auto_unlock", unlock);
