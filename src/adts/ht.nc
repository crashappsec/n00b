...
hash_object(n00b_dict2_t *d, void *key)
{
    ...
    if (n00b_in_heap(key)) {
        ...
        if (h->guard == n00b_gc_guard) {
            if (h->cached_hash) {
                return h->cached_hash;
            }
        }
        else {
            h = NULL; // don't try to cache.
        }
    }

    __int128_t result = (*d->fn)(key);

    if (h) {
        h->cached_hash = result;
    }
...
new_dict_store(n00b_dict2_t *d, uint32_t alloc_items)
{
...
static bool
dict_lock(n00b_dict2_t *d, bool try, uint32_t *count)
{
...
static void
dict_unlock_post_migrate(n00b_dict2_t *d, n00b_dict_store_t *s)
{
    if (d->use_mmap) {
...
static void
dict_unlock_post_copy(n00b_dict2_t *d)
{
    n00b_dict_store_t *s = atomic_read(&d->store);
...
static void
n00b_dict_migrate(n00b_dict2_t *d)
{
    uint32_t            nitems = 0;
    ...
    atomic_store(&news->info.used_count, nitems);
...
n00b_dict2_raw_view(n00b_dict2_t *d, uint32_t *n)
{
...
n00b_list_t *
_n00b_dict2_keys(n00b_dict2_t *d, ...)
{
    keywords
    {
        bool sort = true;
    ...
    uint32_t              n;
    n00b_dict_raw_item_t *arr       = n00b_dict2_raw_view(d, &n);
...
n00b_list_t *
_n00b_dict2_values(n00b_dict2_t *d, ...)
{
    keywords
    {
        bool sort = true;
...
n00b_list_t *
_n00b_dict_items(n00b_dict2_t *d, ...)
{
    keywords
    {
        bool sort = true;
...
n00b_acquire_if_present(n00b_dict2_t      *d,
...
try_again:
        n00b_futex_wait_for_value(&d->futex, ~0);
        store = store->info.next_store;
    } while (true);
}

static inline n00b_dict_bucket_t *
n00b_acquire_or_add(n00b_dict2_t *d, n00b_dict_store_t *store, __int128_t hv)
{
    uint32_t            last_slot;
    uint32_t            bix;
    uint32_t            flags;
...
void *
_n00b_dict2_put(n00b_dict2_t *d, void *key, void *value)
{
...
try_again:
    ...
    if (!bucket->hv) {
        if (atomic_fetch_add(&store->info.used_count, 1) >= store->threshold) {
    ...
    if (reset_epoch) {
        bucket->insert_order
            = (uint32_t)atomic_fetch_add(&store->info.used_count, 1);
    }
...
void *
_n00b_dict2_get(n00b_dict2_t *d, void *key, bool *found)
{
    __int128_t          hv    = hash_object(d, key);
    n00b_dict_store_t  *store = atomic_read(&d->store);
...
bool
_n00b_dict2_replace(n00b_dict2_t *d, void *key, void *value)
{
    __int128_t          hv    = hash_object(d, key);
    n00b_dict_store_t  *store = atomic_read(&d->store);
    n00b_dict_bucket_t *b     = n00b_acquire_if_present(d, store, hv);
...
bool
_n00b_dict2_add(n00b_dict2_t *d, void *key, void *value)
{
    __int128_t         hv    = hash_object(d, key);
    n00b_dict_store_t *store = atomic_read(&d->store);
try_again:
    ...
    bucket->insert_order = (uint32_t)atomic_fetch_add(&store->info.used_count,
                                                      1);
...
bool
_n00b_dict2_remove(n00b_dict2_t *d, void *key)
{
    __int128_t          hv    = hash_object(d, key);
    n00b_dict_store_t  *store = atomic_read(&d->store);
    n00b_dict_bucket_t *b     = n00b_acquire_if_present(d, store, hv);
...
bool
_n00b_dict2_cas(n00b_dict2_t *d,
                void         *key,
                void         *old_item,
                void         *new_item,
                bool          expect_empty,
...
{
    if (expect_empty && delete_existing) {
        return false;
    }
...
try_again:
        ...
        if (!bucket_deleted(b)) {
            if (atomic_fetch_add(&store->info.used_count,
                                 1)
                >= store->threshold) {
                unlock_bucket(b);
                n00b_dict_migrate(d);
        ...
        b->flags &= ~HT_FLAG_DELETED;
        b->value        = new_item;
        b->insert_order = (uint32_t)atomic_fetch_add(&store->info.used_count,
                                                     1);
        unlock_bucket(b);
...
void
n00b_dict2_init(n00b_dict2_t *dict,
                uint32_t      n_buckets,
                bool          use_mmap,
                bool          cstring)
{
    if (n_buckets < N00B_DICT_MIN_SIZE) {
        n_buckets = N00B_DICT_MIN_SIZE;
    }
    else {
        uint32_t bitdiff = (n_buckets & (n_buckets - 1));

        if (bitdiff) {
            n_buckets = 1 << (n00b_int_log2(n_buckets) + 1);
        }
    }

    dict->use_mmap = use_mmap;
    dict->store    = new_dict_store(dict, n_buckets);

    if (cstring) {
        dict->fn = (void *)cstring_hash;
        return;
    }
    if (!n00b_in_heap(dict)) {
        dict->fn = (void *)word_hash;
        return;
    }

    n00b_type_t *ktype = n00b_type_get_param(n00b_get_my_type(dict), 0);
    ...
    // Special case strings since they're so common.
    if (n00b_type_is_string(ktype)) {
        dict->fn = (void *)n00b_string_hash;
        return;
    }
    ...
    vt = (void *)&n00b_base_type_info[ktype->base_index].vtable;
    fn = (void *)vt->methods[N00B_BI_HASH];
