...
extern n00b_stream_t *_n00b_new_fd_channel(n00b_fd_stream_t *fd, ...);
extern n00b_stream_t *_n00b_channel_open_file(n00b_string_t *filename, ...);
extern n00b_stream_t *n00b_channel_fd_open(int);
...
extern n00b_stream_t *_n00b_channel_connect(n00b_net_addr_t *, ...);

#define n00b_new_fd_channel(fdstrm, ...) \
    _n00b_new_fd_channel(fdstrm, N00B_VA(__VA_ARGS__));

#define n00b_channel_open_file(fname, ...) \
    _n00b_channel_open_file(fname, N00B_VA(n00b_kw(__VA_ARGS__)))

#define n00b_create_listener(addr, ...) \
    _n00b_create_listener(addr, N00B_VA(__VA_ARGS__))
#define n00b_channel_connect(addr, ...) \
    _n00b_channel_connect(addr, N00B_VA(__VA_ARGS__))

static inline int
n00b_channel_fileno(n00b_stream_t *c)
{
    if (!c->fd_backed) {
        return -1;
    }

    n00b_fd_cookie_t *cookie = (void *)c->cookie;
    return cookie->stream->fd;
}

#ifdef N00B_USE_INTERNAL_API
extern bool n00b_fdchan_close(n00b_stream_t *);
extern void n00b_fdchan_on_first_subscriber(n00b_stream_t *,
                                            n00b_fd_cookie_t *);
extern void n00b_fdchan_on_no_subscribers(n00b_stream_t *,
                                          n00b_fd_cookie_t *);
#endif
