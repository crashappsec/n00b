...
typedef n00b_list_t *(*n00b_stream_filter_fn)(void *, void *);
...
typedef enum {
    ...
    N00B_STREAM_FILTER_READS,
    // For the other two, any underlying implementation requires
    // something calling n00b_write() or n00b_read().  The
    // filtering occurs before the underlying implementation is
    // called.
    N00B_STREAM_FILTER_WRITES,
} n00b_stream_filter_policy;
...
typedef struct {
    int                   cookie_size;
    n00b_stream_filter_fn setup_fn;
    n00b_stream_filter_fn read_fn;
    ...
    // it's not intended to go to the stream without first going
    ...
    // of the stream, and *should* produce a single type, but it is
...
// json filter
extern n00b_filter_spec_t *n00b_filter_json(bool, bool, bool);
