...
typedef struct {
    n00b_string_t *path;
    n00b_string_t *raw_docstring;
    n00b_string_t *second_entry;
    n00b_string_t *expected_output;
...
extern void                n00b_show_err_diffs(n00b_string_t *,
                                               n00b_list_t *,
                                               n00b_list_t *);
...
{
    n00b_printf("«em4»Running test «b»«#»«/»: «i»«#»",
                (int64_t)item->case_number,
                item->path);
}
...
announce_test_end(n00b_test_kat *kat)
{
    if (kat->run_ok && !kat->exit_code) {
        ...
        n00b_printf(
            "«em4»Finished test «em2»«#»«/»: («#»). "
            "«i»«green»PASSED.",
            (int64_t)kat->case_number,
            kat->path);
    }
    else {
